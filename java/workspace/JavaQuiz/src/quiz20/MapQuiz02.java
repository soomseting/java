			...
			try {
				...
				if(a == 1) {
					...
					if(map.containsKey(menu)) {
						...
						map.put(menu, price);
						System.out.println(menu+"이(가)" + price + "원으로 등록되었습니다.");
//						sc.nextLine();
					}
				}else if(a == 2) {
					 if(map.size() == 0) {
						 ...
						 System.out.println("현재 등록된 메뉴 입니다.");
					...
					System.out.println("수정할 메뉴와 가격을 입력하세요");
					String chM = sc.next();
					Integer chp = sc.nextInt();
					...
					if(map.size() ==0) {
						System.out.println("메뉴판이 비어있습니다.");
					}else {
						System.out.println("등록된 메뉴를 모두 삭제합니다.");
						map.clear();
					}
				}else if(a == 5) {
