...
public class QueueQuiz01 {
	public static void main(String[] args) {
		 ...
		 * 은행 대기열 시뮬레이터
		 * 
		 * Queue 인터페이스와 LinkedList를 활용하여 대기열을 관리하세요. 손님 정보는 Customer라는 클래스로 생성하세요
		 * Customer은 name, arrivalTime을 가지고 있습니다. arrivalTime은 업무처리 소요시간 입니다 (1~3랜덤값을
		 * 초기값으로 주면 됩니다)
		 * 
		 * 손님은 60%의 확률로 도착하고 (랜덤.nextInt(100) < 60) 대기열이 비어있지 않을 때, 50%확률로 업무가 처리됩니다.
		 * (랜덤.nextInt(100) < 50)
		 * 
		...
		for (int i = 1; i <= 30; i++) {
			...
			if (!queue.isEmpty() && ran.nextInt(100) < 50) {
				...
				System.out.println(c.getName() + "의 업무 처리됨.");
			}
